...
from vidstab import VidStab  # 防止视频抖动
import cv2
import numpy as np
import torch
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        #### siam
        from siamfcpp_track.videoanalyst.config.config import cfg as root_cfg
        from siamfcpp_track.videoanalyst.config.config import specify_task
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                #     draw_boxes(im0, bbox_xyxy, identities, categories, None, names)
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=r'./yolov5s.pt', help='model path(s)')
